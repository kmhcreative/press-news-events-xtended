...
class PNE_News extends PNE_Custom_Post_Type {
	...
	function options($post) {
		...
		extract(array(
			'link' => isset($meta['_link'][0]) ? $meta['_link'][0] : '',
			'date' => isset($meta['_date'][0]) ? $meta['_date'][0] : '',
		));
		...
		<table class="pne_news_options">
			<tr>
				<td>
				<label><?php _e("Link:", 'press-news-events'); ?></label></td>
				<td>
					<input
						type="text"
						name="pne_news[link]"
...
// This actually returns the date entered from the date picker:
		if ($date && in_array('date', $show)) $pieces[] = date('l, jS F Y',$date);
